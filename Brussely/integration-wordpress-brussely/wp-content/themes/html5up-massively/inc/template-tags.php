	...
	function base_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list ) {
				...
				printf( '<span class="cat-links">' . esc_html__( ' Posté dans  %1$s', 'base' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			...
			if ( $tags_list ) {
				...
				printf( '<span class="tags-links">' . esc_html__( ' / Tags :  %1$s', 'base' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
			...
			comments_popup_link(
				sprintf(
					wp_kses(
						...
						__( 'Lâche un comm\' (oui ça fait sooooo 2000\'s) <span class="screen-reader-text"> sur  %s</span>', 'base' ),
						array(
							'span' => array(
								'class' => array(),
							),
